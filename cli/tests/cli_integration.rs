...
mod backlinks {
    ...
    fn finds_backlinks_to_topic_b() {
        ...
        let (stdout, _, success) =
            run_hypha(&["--root", fixtures.to_str().unwrap(), "backlinks", "topic-b"]);

        assert!(success);
        assert!(
            stdout.contains("Topic A"),
            "Should find Topic A as backlink"
        );
        assert!(
            stdout.contains("No Frontmatter"),
            "Should find No Frontmatter as backlink"
        );
        assert!(
            stdout.contains("2 backlink(s) found"),
    ...
    fn finds_backlinks_to_topic_c() {
        let fixtures = fixtures_path();
        let (stdout, _, success) =
            run_hypha(&["--root", fixtures.to_str().unwrap(), "backlinks", "topic-c"]);
        ...
        assert!(
            stdout.contains("1 backlink(s) found"),
    ...
    fn orphan_has_no_backlinks() {
        let fixtures = fixtures_path();
        let (stdout, _, success) =
            run_hypha(&["--root", fixtures.to_str().unwrap(), "backlinks", "orphan"]);

        assert!(success);
        assert!(
            stdout.contains("No backlinks found"),
            "Orphan should have no backlinks"
        );
    }
...
mod lint {
    ...
    fn finds_broken_links() {
        let fixtures = fixtures_path();
        let (stdout, _, success) = run_hypha(&["--root", fixtures.to_str().unwrap(), "lint"]);
    ...
    fn lint_json_output() {
        let fixtures = fixtures_path();
        let (stdout, _, _) = run_hypha(&["--root", fixtures.to_str().unwrap(), "lint", "--json"]);
        ...
        assert_eq!(
            warnings.len(),
            2,
            "broken-links.md should have 2 broken link warnings"
        );
    }
    ...
    fn lint_includes_line_and_column() {
        ...
        assert!(
            warning["line"].is_number(),
            "Warning should have line number"
        );
        assert!(
            warning["column"].is_number(),
            "Warning should have column number"
        );
    }
}
...
mod list {
    ...
    fn lists_all_topics() {
        let fixtures = fixtures_path();
        let (stdout, _, success) = run_hypha(&["--root", fixtures.to_str().unwrap(), "list"]);
    ...
    fn list_json_output() {
        let fixtures = fixtures_path();
        let (stdout, _, success) =
            run_hypha(&["--root", fixtures.to_str().unwrap(), "list", "--json"]);
...
mod search {
    ...
    fn search_no_results() {
        ...
        assert!(success);
        assert!(
            stdout.contains("No topics found") || stdout.trim().is_empty() || stdout.contains("0")
        );
    }
}
...
mod info {
    ...
    fn shows_workspace_info() {
        let fixtures = fixtures_path();
        let (stdout, _, success) = run_hypha(&["--root", fixtures.to_str().unwrap(), "info"]);
    ...
    fn info_verbose_shows_field_details() {
        let fixtures = fixtures_path();
        let (stdout, _, success) =
            run_hypha(&["--root", fixtures.to_str().unwrap(), "info", "--verbose"]);
...
mod rename {
    ...
    fn renames_topic() {
        ...
        assert!(
            temp.path().join("renamed.md").exists(),
...
mod edge_cases {
    ...
    fn handles_topic_without_frontmatter() {
        ...
        assert!(
            stdout.contains("No Frontmatter"),
            "Topic without frontmatter should be listed"
        );
    }
    ...
    fn backlinks_match_by_filename_stem() {
        ...
        let (stdout, _, success) = run_hypha(&[
            ...
            fixtures.to_str().unwrap(),
            "backlinks",
            "Topic B", // Using title, not filename
        ]);
